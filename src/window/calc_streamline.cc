...
void Window::calc_streamline(size_t x, size_t y, size_t z)
{
    ...
    for (size_t idx = z; idx != d_simulation.gridsize(); ++idx)
    {
        ...
        newX = curX - d_simulation.gridsize() * (1 + d_simulation.timestep() * d_simulation.history_x()[idx][curX + curY * d_simulation.gridsize()]);
        newY = curY - d_simulation.gridsize() * (1 + d_simulation.timestep() * d_simulation.history_y()[idx][curX + curY * d_simulation.gridsize()]);
